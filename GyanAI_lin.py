"""
GyanAI is open-source, and it uses the replicate model. This is the main file (made for Linux) with the UI and function calls. Please feel free to contribute changes. 
...
def api_keys_add():
    ...
    heading = tk.Label(window, text="Add API Keys", font=("Segoe UI", 24))
    heading.pack()

    label = tk.Label(window, text="Please add a Replicate and OpenAI API key.")
    label.pack()

    rKeyHint = tk.Label(window, text="Enter Replicate API Key")
    rKeyHint.pack()
    rkeyVar = tk.StringVar()
    rKey = tk.Entry(window, textvariable=rkeyVar)
    rKey.pack()

    oKeyHint = tk.Label(window, text="Enter OpenAI API Key")
    oKeyHint.pack()
    okeyVar = tk.StringVar()
    oKey = tk.Entry(window, textvariable=okeyVar)
    oKey.pack()
    def add_keys():
    ...
    submit_btn = tk.Button(window, text="Add API Keys", command=add_keys)
    submit_btn.pack()
